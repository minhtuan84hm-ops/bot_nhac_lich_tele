...
// ─── Claude parser: extract event from natural language ───────────────────────
async function parseEventFromText(text, userId, username) {
  const now = new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
  const prompt = `Bạn là trợ lý phân tích lịch. Hôm nay là ${now} (múi giờ Việt Nam).

Từ đoạn text sau, hãy trích xuất thông tin lịch/sự kiện và trả về JSON (không markdown, không giải thích):
"${text}"
...
- Nếu không có ngày cụ thể và là lệnh tạo lịch, đặt datetime = null
- Nếu là /today hoặc "lịch hôm nay" thì action = "today"
- Nếu là /list hoặc "xem lịch" thì action = "list"
...
function scheduleJob(event) {
  ...
  schedule.scheduleJob(jobId, rule, async () => {
    ...
    // auto-delete one-time events after firing
...
function restoreJobs() {
  ...
  console.log(`✅ Restored ${count} scheduled jobs`);
}
...
async function handleMessage(msg) {
  ...
  try {
    ...
    const parsed = await parseEventFromText(text, userId, username);
    ...
    if (parsed.action === 'delete') {
      return bot.sendMessage(chatId, '🗑 Để xóa lịch, dùng /list rồi nhấn nút xóa tương ứng.');
    }

    if (parsed.action === 'create') {
      if (!parsed.datetime) {
        return bot.sendMessage(chatId,
          `⚠️ Tao chưa rõ *thời gian* của sự kiện này.\n\nVí dụ: _"họp team 2h chiều mai"_ hoặc _"nhắc ${username} lúc 9h sáng thứ 2"_`,
          { parse_mode: 'Markdown' });
      }
...
function handleList(chatId) {
  ...
  const lines = events.slice(0, 10).map((ev, i) => {
    const dt = new Date(ev.datetime);
    return `${i + 1}. *${ev.title}*\n   🕐 ${formatDateTime(dt)} ${repeatLabel(ev.repeat)}`;
  }).join('\n\n');
  bot.sendMessage(chatId,
    `📋 *Lịch sắp tới (${events.length}):*\n\n${lines}\n\n_Để xóa lịch, reply tin nhắn này với "xóa [số]"_`,
    { parse_mode: 'Markdown' });
}

function helpText() {
  return `👋 *Tao là bot trợ lý lịch!*

Chỉ cần nhắn tin tự nhiên, tao hiểu:
...
bot.on('message', (msg) => {
  if (msg.text && !msg.text.startsWith('/')) handleMessage(msg);
  else if (msg.text === '/start') handleMessage(msg);
});
